...
- package-authored handoff gates loaded through `Docs/ContentPackageLoader.md`
...
The built-in fallback gates connect Stonewake Vale and Brindlebrook Roadlands through AmandaCore-owned transition IDs. The package loader can add validated content-authored gates, such as the Dawnwake Landing <-> Tideglass Shoal validation route.
...
`zone-package-handoff-basic` follows the same runtime path but starts from validated package content. It confirms package zones are registered, package gates are available to the handoff coordinator, and clients complete package-defined zone transitions.
...
- Built-in transition gates still exist as fallback dev content. Dawnwake package gates are loaded from content manifests, but broader authored gate tooling is future work.
