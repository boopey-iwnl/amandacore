...
$character = Invoke-JsonPost "http://localhost:8084/v1/characters" @{ realmId = $realmId; displayName = "Runner$($username.Substring($username.Length - 4))"; archetypeId = "wayfarer_warden" } $token
...
$loginAfterClient = Invoke-JsonPost "http://localhost:8081/v1/auth/login" @{ username = $username; password = $password } $null
$tokenAfterClient = $loginAfterClient.accessToken
$ticketAfterClient = Invoke-JsonPost "http://localhost:8085/v1/world/join-ticket" @{ realmId = $realmId; characterId = $character.id } $tokenAfterClient
$connectAfterClient = Invoke-JsonPost "http://localhost:8085/v1/world/connect" @{ ticketId = $ticketAfterClient.ticketId } $null
$state = Invoke-JsonGet ("http://localhost:8085/v1/world/state?worldSessionToken=" + $connectAfterClient.worldSessionToken) $null
