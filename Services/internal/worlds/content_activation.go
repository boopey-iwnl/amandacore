...
func contentZoneTransitions(zone contentpkg.ZoneDefinition) []zonePointDefinition {
	points := make([]zonePointDefinition, 0, len(zone.TransitionGates))
	for _, gate := range zone.TransitionGates {
		points = append(points, zonePointDefinition{
			ID:          gate.TransitionID,
			DisplayName: gate.TransitionID,
			Type:        strings.ToLower(strings.TrimSpace(gate.Kind)),
