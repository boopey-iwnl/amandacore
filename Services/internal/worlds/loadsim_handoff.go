...
func RunZoneHandoffLoadsim(opts ZoneHandoffLoadsimOptions) (ZoneHandoffLoadsimReport, error) {
	...
	if opts.Scenario == "" {
		opts.Scenario = "zone-handoff-basic"
	}
	...
	if opts.Scenario != "zone-handoff-basic" && opts.Scenario != "zone-package-handoff-basic" {
		return report, fmt.Errorf("unsupported loadsim scenario %q", opts.Scenario)
	}

	if opts.Scenario == "zone-package-handoff-basic" {
	...
	server := newWorldServerWithContentPackage(fileStore, opts.ContentPath)
	if opts.Scenario == "zone-package-handoff-basic" {
		if server.contentRegistry == nil {
			err := fmt.Errorf("content package did not activate")
			report.Errors = append(report.Errors, err.Error())
			return report, err
		}
		report.PackageID = server.contentRegistry.PackageID
		report.ContentPackageLoaded = len(server.contentActivation.Errors) == 0
		report.ZonesActivated = server.contentActivation.ZonesActivated
		report.HandoffGatesRegistered = server.contentActivation.HandoffGatesRegistered
		for _, validationError := range server.contentActivation.Errors {
			report.ValidationErrors = append(report.ValidationErrors, fmt.Sprintf("%s at %s: %s", validationError.Code, validationError.Path, validationError.Message))
		}
		if !report.ContentPackageLoaded {
			err := fmt.Errorf("content package activation failed")
			report.Errors = append(report.Errors, err.Error())
			return report, err
		}
	}
	...
	for clientIndex := 0; clientIndex < opts.Clients; clientIndex++ {
		startedAt := time.Now()
		if err := runZoneHandoffLoadsimClient(server, fileStore, clientIndex, opts.TransitionLoops, opts.Scenario, &report); err != nil {
			report.Errors = append(report.Errors, err.Error())
		}
...
func runZoneHandoffLoadsimClient(server *worldServer, fileStore *store.FileStore, clientIndex int, transitionLoops int, scenario string, report *ZoneHandoffLoadsimReport) error {
	...
	startGate, err := firstHandoffGateForScenario(server, scenario)
	if err != nil {
		return err
	}
	...
	for step := 0; step < transitionLoops; step++ {
		gate, err := firstHandoffGateFromZone(server, session.ZoneID)
		if err != nil {
			return fmt.Errorf("client %d handoff %d route failed: %w", clientIndex+1, step+1, err)
		}
		session.X = gate.GateX
		session.Y = gate.GateY
...
func firstHandoffGateForScenario(server *worldServer, scenario string) (ZoneHandoffGateDefinition, error) {
	if scenario == "zone-package-handoff-basic" && server != nil && server.contentRegistry != nil {
		for _, gateID := range contentpkg.SortedKeys(server.contentRegistry.HandoffGates) {
			gate := server.handoffGateLocked(gateID)
			if gate.TransitionID != "" && gate.Enabled {
				return gate, nil
			}
		}
		return ZoneHandoffGateDefinition{}, fmt.Errorf("content package has no active handoff gates")
	}
	return server.handoffGateLocked("to_brindlebrook"), nil
}

func firstHandoffGateFromZone(server *worldServer, zoneID string) (ZoneHandoffGateDefinition, error) {
	gates := server.zoneHandoffGatesForZoneLocked(zoneID)
	for _, gate := range gates {
		if gate.Enabled {
			return gate, nil
		}
	}
	return ZoneHandoffGateDefinition{}, fmt.Errorf("zone %q has no active handoff gate", zoneID)
}
