...
func TestBuildZoneShardAssignmentsStableAcrossSortedZones(t *testing.T) {
	...
	assignments, err := BuildZoneShardAssignments(registry, ShardAssignmentPolicy{ShardCount: 2})
	if err != nil {
	...
	if assignments["amberglass_fields"].Shard != "zone_shard_01" {
		t.Fatalf("expected first sorted zone on zone_shard_01, got %#v", assignments["amberglass_fields"])
	}
	if assignments["dawnwake_landing"].Shard != "zone_shard_02" {
