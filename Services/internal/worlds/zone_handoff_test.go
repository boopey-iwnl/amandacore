...
func TestZonePackageHandoffLoadsimUsesPackageLoadedGates(t *testing.T) {
	report, err := RunZoneHandoffLoadsim(ZoneHandoffLoadsimOptions{
		Clients:         3,
		Duration:        100 * time.Millisecond,
		CmdRate:         4,
		...
		Shards:          2,
		QueueCapacity:   8,
		Scenario:        "zone-package-handoff-basic",
		ContentPath:     contentpkg.DawnwakePackagePath,
	})
	if err != nil {
		t.Fatalf("loadsim failed: %v; report=%#v", err, report)
	}
	if len(report.Errors) > 0 || len(report.ValidationErrors) > 0 {
		t.Fatalf("unexpected loadsim errors: %#v %#v", report.Errors, report.ValidationErrors)
	}
	if !report.ContentPackageLoaded || report.PackageID != "dawnwake_isles_foundation" {
		t.Fatalf("expected Dawnwake package to load, got %#v", report)
	}
	if report.HandoffGatesRegistered != 2 {
		t.Fatalf("expected two package gates, got %d", report.HandoffGatesRegistered)
	}
	if report.HandoffsCompleted != 6 {
		t.Fatalf("expected 6 completed handoffs, got %d", report.HandoffsCompleted)
	}
}
